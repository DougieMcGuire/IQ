// dopamine-loader.js — Drop this in same directory, add <script src="dopamine-loader.js"></script> before </head>
(function(){
  var s = document.createElement('style');
  s.id = 'dopamine-patch';
  s.textContent = `/* ══════════════════════════════════════════════════════════════
   CEBEAR DOPAMINE PATCH — Drop this AFTER all existing CSS 
   in improve.html (before </style> closing tag)
   ══════════════════════════════════════════════════════════════ */
...
/* ── REACTION — bigger circle, juicier ── */
...
.conn-word{
  ...
  border:2.5px solid #e8e2d6!important;border-radius:12px!important;
  box-shadow:0 4px 0 #d6cfc0!important;
  padding:clamp(11px,2.8vw,15px) 6px!important;
  font-size:clamp(10px,2.5vw,12px)!important;
  transition:all .12s cubic-bezier(.34,1.56,.64,1)!important;
}
.conn-word:active{transform:translateY(4px)!important;box-shadow:none!important}
.conn-word.conn-sel{
  background:#e8f2ff!important;border-color:var(--blue)!important;color:var(--blue)!important;
  box-shadow:0 4px 0 var(--blue-dark)!important;transform:translateY(-3px) scale(1.03)!important;
}
.conn-word.conn-shake{animation:wrongShake .5s ease!important}
.conn-cat{
  border-radius:14px!important;padding:12px 10px!important;
  animation:popBounce .4s cubic-bezier(.34,1.56,.64,1)!important;
...
.conn-sub{
  background:#fff!important;color:var(--text)!important;
  border:2.5px solid #e8e2d6!important;border-radius:14px!important;
  box-shadow:0 4px 0 #d6cfc0!important;padding:11px 30px!important;
...
/* ── WATER SORT — fatter tubes, better colors ── */
...
/* ── CATEGORY PILL — better look ── */
.category{
  animation:popBounce .3s ease-out!important;
  box-shadow:0 2px 0 rgba(0,0,0,.1)!important;
}

/* ── MATH RUSH — timer urgency ── */
